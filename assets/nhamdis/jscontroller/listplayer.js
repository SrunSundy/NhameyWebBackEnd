		...
		function getCountPlayer(){
	    	...
	    	$.ajax({
	    		type : "GET",
	    		url : $("#base_url").val()+"API/PlayerRestController/getCountPlayer",
	  
	    		success : function(data){

	    			data = JSON.parse(data);
		    		$("#disable_items").html(data.player_data.disactive_player);
					$("#enable_items").html(data.player_data.active_player);
					$("#total_items").html(data.player_data.total_player);
	    ...
	    function loadShopDataToTable(){
	    	...
	    	$.ajax({
	    		type : "POST",
	    		url : $("#base_url").val()+"API/PlayerRestController/listPlayer",
	    ...
	    function updateShopStatus( status , userid , callback){

		   	progressbar.start();   	
	    	$.ajax({
				type : "POST",
				url : $("#base_url").val()+"API/PlayerRestController/togglePlayer",
				contentType : "application/json",
				data :  JSON.stringify({
					"resq_data" : {
						"user_id" : userid,
		...
		function addSrcLogoimg( image ){
			return $("#dis_img_path").val()+"/uploadimages/real/user/small/"+image;
		}
		...
		$(document).on("click", ".toggleshop", function(){
			...
			if($(this).is(":checked")){
				...
				updateShopStatus( 1 ,shopid , function(){
					...
					swal("Player is updated!", "This Player will be visible for clients", "success"); 
				});
				/* $(this).prop('checked', false);				
				...
				swal({
					  title: "Are you sure?",
					  text: "The client will not be able to see this Player",
					  type: "warning",
					  showCancelButton: true,
					  confirmButtonColor: "#DD6B55",
				 ...
				 function(isConfirm) {
			        if (isConfirm) {				       			        	
			        	updateShopStatus( 0 ,shopid , function(){
				        	...
				        	swal("Player is updated!", "The client will not be able to view this Player", "success"); 
				        });					 
			        } else {
