	    ...
	    function loadShopDataToTable(){
	    	...
	    	$.ajax({
	    		type : "POST",
	    		url : $("#base_url").val()+"API/PlayerPostRestController/listPlayerPost",
	    ...
	    function updateShopStatus( status , userid , callback){
	    	...
	    	$.ajax({
				type : "POST",
				url : $("#base_url").val()+"API/PlayerPostRestController/togglePlayerPost",
				contentType : "application/json",
				data :  JSON.stringify({
					"resq_data" : {
						"user_id" : userid,
		...
		function addSrcLogoimg( image ){
			return $("#dis_img_path").val()+"/uploadimages/real/user/medium/"+image;
		}
