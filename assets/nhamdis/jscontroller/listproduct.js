	    ...
	    function loadShopDataToTable(){
	    	...
	    	$.ajax({
	    		type : "POST",
	    		url : $("#base_url").val()+"API/ProductRestController/listProduct",
	    ...
	    function updateShopStatus( status , shopid , callback){
	    	...
	    	$.ajax({
				type : "POST",
				url : $("#base_url").val()+"API/ProductRestController/toggleProduct",
				contentType : "application/json",
				data :  JSON.stringify({
					"resq_data" : {
						"shop_id" : shopid,
		...
		function addSrcLogoimg( image ){
			return $("#dis_img_path").val()+"/uploadimages/real/product/small/"+image;
		}
