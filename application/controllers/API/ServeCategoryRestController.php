...
class ServeCategoryRestController extends CI_Controller{
	...
	public function getServeCategoryByShopId($shop_id){
		
		$shop_id  = urlencode($shop_id);
		$data = $this->ServeCategoryModel->getServeCategoryByShopId($shop_id);
		$json = json_encode($data);
		echo $json;
	...
	public function insertServeCategory(){
		...
		$servecategoryarr = array(trim($servecategorydata['serve_category_name']), $servecategorydata['serve_category_type'], $servecategorydata['serve_category_icon'] ,$servecategorydata['serve_category_remark'], 1);
