...
class ProductRestController extends CI_Controller{	
    ...
    public function getDefaultUpdateInfo(){
		...
		$response = $this->ProductModel->getDefaultUpdateInfo($request);
	...
	public function getCountProduct(){
		...
		$response = $this->ProductModel->getCountProduct();
		$json = json_encode($response, JSON_PRETTY_PRINT);
		echo $json;
	}
	...
	public function updateProductPrice(){
		...
		$shopdata = $shopdata["product_data"];
		
		$response = $this->ProductModel->updateProductPrice($shopdata);
		echo json_encode($response);
		
	}
	public function updateProductServeCateogry(){
		
		$product_data = json_decode($this->input->raw_input_stream,true);
		$product_data = $product_data["product_data"];
		
		$response = $this->ProductModel->updateProductServeCateogry($product_data);
		
		echo json_encode($response);
	}
	public function updateProductTage(){
		
		$product_data = json_decode($this->input->raw_input_stream,true);
		$product_data = $product_data["product_data"];
		
		$response = $this->ProductModel->updateProductTage($product_data);
		
		echo json_encode($response);
	}
	public function getTageByProId($proid){
		
		$proid  = urlencode($proid);
		$this->load->model("TagModel");
		
		$data = $this->TagModel->getTageByProId($proid);
