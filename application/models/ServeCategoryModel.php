...
class ServeCategoryModel extends CI_Model{
	...
	function getServeCategoryByName( $cate , $limit ,  $status){
    	...
    	$sql = "SELECT serve_category_id,serve_category_name from nham_serve_category 
	...
	function insertServeCategory( $servecatearr ){
		...
		$this->db->trans_start();
		$query = $this->db->query('INSERT INTO nham_serve_category(serve_category_name, serve_category_remark) values (? , ?)', $servecatearr);
