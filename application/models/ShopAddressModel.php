...
class ShopAddressModel extends CI_Model{
	...
	public function getShopAddressByNameCombo($address_type, $srchname, $parent_id , $limit){
		
		if($address_type=='country'){
			$sql = "SELECT country_id, country_name FROM nham_country
				WHERE country_name LIKE ? AND country_status>? AND country_status=1
				ORDER BY country_id DESC
				limit ?";
		}else if($address_type=='city'){
			$sql = "SELECT city_id, city_name FROM nham_city
				WHERE city_name LIKE ? AND country_id=? AND city_status=1
				ORDER BY city_id DESC
				limit ?";
		}else if($address_type=='district'){
			$sql = "SELECT district_id, district_name FROM nham_district
				WHERE district_name LIKE ? AND city_id=? AND district_status=1
				ORDER BY district_id DESC
				limit ?";
		}else if($address_type=='commune'){
			$sql = "SELECT commune_id, commune_name FROM nham_commune
				WHERE commune_name LIKE ? AND district_id=? AND commune_status=1
	...
	public function updateShopAddress($req_data){
		...
		if($this->IsNullOrEmptyString($validate) && $this->IsNullOrEmptyString($validate2) && $this->IsNullOrEmptyString($validate3) && $this->IsNullOrEmptyString($validate5) && $this->IsNullOrEmptyString($validate6) && $this->IsNullOrEmptyString($validate6)){
			if($address_type=='country'){
				$strsql = "UPDATE nham_country set country_name=?, admin_id=?, country_status=? where country_id=?";
				$params = array($data_name, $admin_id, $status, $id);
			}else if($address_type=='city'){
				$strsql = "UPDATE nham_city set city_name=?, admin_id=?, city_status=? where city_id=? and country_id=?";
				$params = array($data_name, $admin_id, $status, $id, $parent_id);
			}else if($address_type=='district'){
				$strsql = "UPDATE nham_district set district_name=?, admin_id=?, district_status=? where district_id=? and city_id=?";
				$params = array($data_name, $admin_id, $status, $id, $parent_id);
			}else if($address_type=='commune'){
				$strsql = "UPDATE nham_commune set commune_name=?, admin_id=?, commune_status=? where commune_id=? and district_id=?";
				$params = array($data_name, $admin_id, $status, $id, $parent_id);
			}
	...
	public function getListShopAddressByNameCombo($address_type, $srchname, $parent_id , $limit){
		if($address_type=='country'){
			$sql = "SELECT country_id, country_name, country_status FROM nham_country
				WHERE country_name LIKE ? AND country_id > ? 
				ORDER BY country_id DESC 
				limit ?";
		}else if($address_type=='city'){
			$sql = "SELECT city_id, city_name, city_status FROM nham_city
				WHERE city_name LIKE ? AND country_id=? 
				ORDER BY city_id DESC
				limit ?";
		}else if($address_type=='district'){
			$sql = "SELECT district_id, district_name, district_status FROM nham_district
				WHERE district_name LIKE ? AND city_id=? 
				ORDER BY district_id DESC
				limit ?";
		}else if($address_type=='commune'){
			$sql = "SELECT commune_id, commune_name, commune_status FROM nham_commune
